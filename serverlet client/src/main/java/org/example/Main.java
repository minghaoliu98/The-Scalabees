...
public class Main {
    //for serverlet
    private static String url = "http://54.191.42.110:8080/A1_server_war/";
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        for (int numThread = MIN_THREAD; numThread <= MAX_THREAD + 1; numThread += 50){
            ...
            printGetRequestResult(req.latencyGet);
            progress.await();
        }
    }
    ...
    private static void printGetRequestResult(List<Integer> latency) {
        Collections.sort(latency);
        long sum = 0;
        int min = Integer.MAX_VALUE;
        int max = 0;
        ...
        double medium = (double) (latency.get(latency.size()/2) + latency.get(latency.size()/2 - 1)) / 2.0;
        System.out.println("--------------------------------------------------");
        System.out.println("GetRequest Result");
        System.out.println("Mean latency " + mean + " || Medium latency" + medium);
        System.out.println("Min latency " + min + " || Max latency " + max);
