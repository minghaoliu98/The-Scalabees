...
public class PostRequest {
    ...
    public List<Integer> latencyGet = new ArrayList<>();
        ...
        Runnable test = new Runnable() {
            ...
            public void run() {
                ...
                for(int i = 0; i < runs; i++){
                    ...
                    final HttpPost httpPost = new HttpPost(URL+"swipe/leftorright/");
                    try {
            ...
            public void run() {
                while (completed.getCount() >= 1) {
                    for (int i = 0; i < 5; i++) {
                        ...
                        int swipee = randInt(1, 1000000);
                        GetMethod method = new GetMethod(URL + action + swipee);
                        try {
                            client.executeMethod(method);
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                        int l = (int)(System.currentTimeMillis() - start);
                        latencyGet.add(l);
                    }
			  try {
                        Thread.sleep(700);
